...
export default function QuizView({ srs = {}, known, onRate, themeColor = '#c77dff', godMode, studyMode = 'flip_es_en', voices = [] }) {
  ...
  const answer = useCallback((type) => {
    ...
    if (yes) setEasy(e => e + 1);
      ...
      setAgain(a => a + 1);
      // put card back at end of queue for re-review
  ...
  // category label — find which category this card belongs to
  const catLabel = (() => {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100%', padding:'12px 14px 4px' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:14 }}>
        ...
        <span style={{ color:C.dim, fontSize:11, minWidth:44, textAlign:'right', fontFamily:"'Space Mono',monospace" }}>{idx + 1}/{total}</span>
      </div>
      ...
      <div
        ...
        style={{
          ...
          border:`1.5px solid ${tc}${flipped ? 'aa' : '33'}`,
          borderRadius:24, padding:'28px 24px',
          display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center',
          cursor: flipped ? 'default' : 'pointer',
          userSelect:'none',
          boxShadow: flipped ? `0 0 28px ${tc}33,inset 0 0 40px ${tc}11` : `0 0 16px ${tc}18`,
      ...
      {/* Session stats */}
      <div style={{ display:'flex', justifyContent:'center', gap:24, paddingBottom:10, paddingTop:8 }}>
