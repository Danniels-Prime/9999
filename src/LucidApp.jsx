...
const GLOBAL_CSS = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  .lang-card:hover{border-color:var(--cc);transform:translateY(-4px) scale(1.02);box-shadow:0 16px 40px rgba(0,0,0,0.5),0 0 0 1px var(--cc)30}
    ...
    const draw = () => {
      if (warp) {
        ...
        stars.forEach(s => {
          ...
          const x1 = cx + Math.cos(s.angle) * pd, y1 = cy + Math.sin(s.angle) * pd;
          const x2 = cx + Math.cos(s.angle) * cd, y2 = cy + Math.sin(s.angle) * cd;
          const alpha = Math.min(s.dist * 1.8, 1);
...
function Nebulae({ color, godMode }) {
  return (
    <>
      <div style={{ position:'fixed',width:'70vw',height:'70vw',borderRadius:'50%',background:`radial-gradient(circle,${color}18 0%,transparent 70%)`,top:'-20%',left:'-15%',filter:'blur(60px)',animation:`nebula-a 22s ease-in-out infinite${godMode ? ',rainbowShift 6s linear infinite' : ''}`,pointerEvents:'none',zIndex:0,transition:'background 0.8s ease' }}/>
      <div style={{ position:'fixed',width:'55vw',height:'55vw',borderRadius:'50%',background:`radial-gradient(circle,${color}12 0%,transparent 65%)`,top:'35%',right:'-15%',filter:'blur(50px)',animation:`nebula-b 28s ease-in-out infinite${godMode ? ',rainbowShift 8s linear infinite' : ''}`,pointerEvents:'none',zIndex:0,transition:'background 0.8s ease' }}/>
      <div style={{ position:'fixed',width:'45vw',height:'45vw',borderRadius:'50%',background:`radial-gradient(circle,${color}0d 0%,transparent 60%)`,bottom:'-15%',left:'25%',filter:'blur(40px)',animation:`nebula-c 19s ease-in-out infinite${godMode ? ',rainbowShift 10s linear infinite' : ''}`,pointerEvents:'none',zIndex:0,transition:'background 0.8s ease' }}/>
    </>
  );
}

/* ── Phrase / Slang Card ── */
function LangCard({ item, theme, isFlipped, onFlip, index, godMode }) {
  return (
    <button
      className={`lang-card${isFlipped ? ' flipped' : ''}${godMode ? ' god-mode' : ''}`}
      style={{ '--cc': godMode ? '#FFD700' : theme.color, '--cg': theme.glow, '--cd': theme.dim, animationDelay: `${index * 0.022}s` }}
      onClick={onFlip}
    >
      <div className="card-glow"/>
      {isFlipped && <div className="ripple-ring"/>}
      <div style={{ fontSize:'11px',color:'#6b69a0',fontWeight:800,letterSpacing:'0.8px' }}>🇪🇸 ES</div>
      <div style={{ fontSize:item.es.length>22?'13px':item.es.length>14?'15px':'18px',fontWeight:900,color:'#FFD700',lineHeight:1.3,textShadow:isFlipped?'0 0 14px #FFD70080':'none',transition:'text-shadow .2s' }}>
        {item.es}
      </div>
      {isFlipped ? (
        <>
          <div style={{ width:'100%',height:'1px',background:'rgba(255,255,255,0.06)',margin:'2px 0' }}/>
          <div style={{ fontSize:'11px',color:godMode?'#FFD700':theme.color,fontWeight:800,letterSpacing:'0.8px' }}>🇺🇸 EN</div>
          <div style={{ fontSize:item.en.length>22?'12px':item.en.length>14?'14px':'16px',fontWeight:900,color:godMode?'#FFD700':theme.color,lineHeight:1.3,textShadow:`0 0 12px ${godMode?'#FFD700':theme.color}` }}>
            {item.en}
          </div>
          {item.meaning && <div style={{ fontSize:'10px',color:'#5e5c88',fontStyle:'italic',lineHeight:1.3 }}>{item.meaning}</div>}
        </>
      ) : (
  ...
  return (
    <div style={{ height:'100%',display:'flex',flexDirection:'column' }}>
      ...
      <div style={{ flexShrink:0,display:'flex',alignItems:'center',gap:'10px',padding:'10px 18px 6px' }}>
        <div style={{ width:'5px',height:'34px',borderRadius:'3px',background:themeColor,boxShadow:`0 0 16px ${themeColor}` }}/>
        <div>
          <h2 style={{ fontSize:'16px',fontWeight:900,color:themeColor,lineHeight:1.2,textShadow:`0 0 12px ${themeColor}50` }}>
            {theme.icon} {theme.label}
          </h2>
          <p style={{ fontSize:'11px',color:'#5e5c88',fontWeight:600 }}>{items.length} cards · {flipped.size} revealed</p>
        </div>
        {flipped.size > 0 && (
          <button onClick={() => setFlipped(new Set())} style={{ marginLeft:'auto',fontSize:'11px',color:'#5e5c88',background:'rgba(255,255,255,0.04)',border:'1px solid #27254a',borderRadius:'20px',padding:'4px 10px',cursor:'pointer',fontWeight:700 }}>
            🔄 Reset
          </button>
        )}
      </div>
      ...
      <main style={{ flex:1,overflowY:'auto',padding:'0 12px 16px' }}>
        <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(140px,1fr))',gap:'10px',maxWidth:'880px',margin:'0 auto' }}>
          {items.map((item, i) => (
            <LangCard key={item.id} item={item} theme={theme} isFlipped={flipped.has(item.id)}
              onFlip={() => toggleFlip(item.id)} index={i} godMode={godMode} />
          ))}
        </div>
      </main>
    </div>
...
export default function LucidApp() {
  ...
  // Persistent state
  const [lifetimeScore, _setLifetimeScore]   = useState(() => LS.get('lucid_xp', 0));
  const [bestEverStreak, _setBestEverStreak] = useState(() => LS.get('lucid_streak', 0));
  const [hideAnswer, _setHideAnswer]         = useState(() => LS.get('lucid_hide', false));

  const setLifetimeScore = v => { _setLifetimeScore(v); LS.set('lucid_xp', v); };
  const setBestEverStreak = v => { _setBestEverStreak(v); LS.set('lucid_streak', v); };
  const toggleHideAnswer = () => _setHideAnswer(prev => { const next = !prev; LS.set('lucid_hide', next); return next; });
    ...
    <div style={{ height:'100%',position:'relative',background:'#080716',color:'#e8e6ff',overflow:'hidden' }}>
      ...
      <div style={{ position:'relative',zIndex:1,height:'100%',display:'flex',flexDirection:'column' }}>
        <div style={{ flex:1,overflow:'hidden',paddingBottom:'62px' }}>
          {view === 'learn'    && <LearnView godMode={godMode} />}
          {view === 'blast'    && (
            <WordBlast
          ...
          {view === 'speak'    && <SpeakCoach themeColor={themeColor} onBack={() => setView('learn')} />}
          {view === 'settings' && (
            <Settings
